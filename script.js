document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    if (!startBtn) { console.error('startBtn not found'); return; }
    ...
    startBtn.addEventListener("click", () => {
      // user gesture; try play (may still be blocked by browser autoplay rules if muted etc)
    ...
    function handleAnswer(correct) {
      ...
      emoji.style.top = (20 + Math.random() * 60) + "vh";
    ...
    function showMemory() {
      ...
      // fallback animation if GSAP not ready
      if (window.gsap) {
        gsap.fromTo(memoryPhoto, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: 0.8 });
        gsap.fromTo(memoryCaption, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.8 });
      } else {
        memoryPhoto.style.opacity = 0;
        setTimeout(() => memoryPhoto.style.opacity = 1, 60);
      }
    }
    ...
    secretBtn.addEventListener("click", () => {
      ...
      if (window.gsap) gsap.fromTo(surprise, { scale: 0.8, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.4 });
    });

    closeSurprise.addEventListener("click", () => {
      if (window.gsap) {
        gsap.to(surprise, { opacity: 0, duration: 0.25, onComplete: () => {
